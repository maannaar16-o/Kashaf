...
def collective_blind(mem, pack):
    ...
    for c in pack.raw["combo"]:
        ...
        # **كل** ما تطلبه التركيبة غائبٌ فعلاً — لا بعضُه. والتقاطع الجزئي
        # يستدعي ثلاث تركيبات على حالة `56-TEAM-P001` بينما التشغيل المعتمد
        # يستدعي **واحدة**؛ فالشرط هو الاحتواء التام، وبه يُستعاد مخرجه.
...
def pair_matrix(mem, pack):
    ...
    **قراءة تشغيلية معلَنة** (`DEC-278`): الميثاق يوجب تقاطعاً لكل زوج ولا
    ينصّ على قاعدة الاختيار؛ والتشغيل التجريبي المعتمد `56-TEAM-P001`
    ينفّذها في ثلاثة صفوف متسقة: **يُقدَّم الزوج القطبي إن وُجد بين
    مهيمنَيهما، وإلا فأعلى مهيمنٍ لكلٍّ**. فالقاعدة **مقروءة من تشغيلٍ
    معتمد لا مخترَعة** — وتُرفع للمالك بوصفها قراءة.
    """
    out = []
